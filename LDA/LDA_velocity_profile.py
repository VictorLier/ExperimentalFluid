...
class LDA_velocity():
    ...
    def _get_data_folders(self) -> list:
        ...
        # sorted_folders = sorted(folders, key=lambda x: int(x.split('_')[-2]))
    ...
    def _get_mean_velocity_all_folders(self) -> None:
        ...
        for folder in self._folders:
            self._get_single_mean_velocity(folder)
    ...
    def plot_velocities(self) -> None:
        ...
        plt.figure()
        plt.errorbar(range(len(velocities)), velocities, yerr=standard_deviations, fmt='.k')
